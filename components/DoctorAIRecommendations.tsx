...
export const AIRecommendation: React.FC<AIRecommendationProps> = ({
  ...
  const getRecommendedDoctors = (): (Doctor & { aiScore?: number })[] => {
    return doctors
      .map(doctor => {
        let score = 0;
        ...
        const reviewCount = doctor.reviewCount || 85; // default since not all have it
        score += Math.min(reviewCount / 100, 1) * 20;
        
        // Experience factor (15% weight)
        score += Math.min(doctor.experience / 20, 1) * 15;
        
        // Availability factor (10% weight)
        const nextAvailable = doctor.nextAvailable || 'Available Now';
        const isAvailableToday = nextAvailable !== 'Available Now' ? 
          new Date(nextAvailable) <= new Date(Date.now() + 24 * 60 * 60 * 1000) : true;
  ...
  const getRecommendationReason = (doctor: Doctor): string => {
    ...
    const reviewCount = doctor.reviewCount || 85;
    if (reviewCount >= 200) return "Highly reviewed by patients";
